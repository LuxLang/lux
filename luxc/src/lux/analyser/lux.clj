...
(defn analyse-module [analyse optimize eval! compile-module ?meta]
  (|do [_ &/ensure-statement
        ...
        _ (&&module/set-anns ==anns current-module)
        ...
        ;; TODO: MUST FIX
        _ (&/fold% (fn [compiler [path _async]]
                     (&/|log! (str "PRE Merge: " path " in " current-module "\n"))
                     (|case @_async
                       (&/$Right _new-compiler)
                       (do (&/|log! (str "POST Merge: " path " in " current-module "\n"))
